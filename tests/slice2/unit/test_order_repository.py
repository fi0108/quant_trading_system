...
from data.storage.models import create_tables, database, drop_tables
...
def test_save_order(clean_orders):
    """测试保存订单"""
    order = Order(
        order_id=12345, symbol="AAPL", action="BUY", order_type="MARKET", quantity=100, status=OrderStatus.SUBMITTED
    )
...
def test_get_by_symbol(clean_orders):
    ...
    OrderRepository.save(
        Order(order_id=1, symbol="AAPL", action="BUY", order_type="MARKET", quantity=100, status=OrderStatus.SUBMITTED)
    )
    OrderRepository.save(
        Order(order_id=2, symbol="TSLA", action="BUY", order_type="MARKET", quantity=50, status=OrderStatus.SUBMITTED)
    )
    OrderRepository.save(
        Order(order_id=3, symbol="AAPL", action="SELL", order_type="MARKET", quantity=50, status=OrderStatus.FILLED)
    )
...
def test_get_by_status(clean_orders):
    ...
    OrderRepository.save(
        Order(order_id=2, symbol="TSLA", action="BUY", order_type="MARKET", quantity=50, status=OrderStatus.FILLED)
    )
    OrderRepository.save(
        Order(order_id=3, symbol="GOOGL", action="BUY", order_type="MARKET", quantity=10, status=OrderStatus.FILLED)
    )
